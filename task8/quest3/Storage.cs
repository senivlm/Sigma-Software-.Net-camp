   ...
   public class Storage
    {
        ...
        private string ReadPlainFromFiles()
        {
            ...
            do
            {
                ...
                FileReader.Path = Console.ReadLine();
                res = FileReader.ReadFromFile();
                i++;
            }
        ...
        #endregion task7
        #region task8
        public static Storage FirstExceptSecond(Storage a, Storage b)
        {
            List<Product>  output = new List<Product>();
            output.AddRange(a.Product_storage);
            foreach (var first in a.Product_storage)
                foreach (var second in b.Product_storage)
                    if (first == second)
                        output.Remove(first);

            Storage res = new Storage(output.ToArray());
            return res;
        }
        public static Storage SameInFirstAndSecond(Storage a, Storage b)
        {
            List<Product> output = new List<Product>();
        ...
        public static Storage SelectAllDistinct(Storage a, Storage b)
        {
            List<Product> output = new List<Product>();
            output.AddRange(a.Product_storage);
            foreach (var first in a.Product_storage)
                foreach (var second in b.Product_storage)
                {
                    ...
                    {
                        if (outputItem == second)
                            notDupe = false;
                    }
                    if (first != second && notDupe)
                        output.Add(second);
                }
